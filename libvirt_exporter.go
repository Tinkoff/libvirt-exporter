...
var (
	...
	libvirtUpDesc = prometheus.NewDesc(
		...
		nil,
		nil)
    libvirtNodeCpu = prometheus.NewDesc(
		prometheus.BuildFQName("libvirt", "", "node_cpu"),
		"Number of CPUs on node.",
		[]string{"host"},
		nil)
	libvirtNodeMemory = prometheus.NewDesc(
		prometheus.BuildFQName("libvirt", "", "node_memory_bytes"),
	...
	libvirtVersionsInfoDesc = prometheus.NewDesc(
		...
		[]string{"host","hypervisor_running", "libvirtd_running", "libvirt_library"},
		nil)
	...
	libvirtDomainMetaBlockDesc = prometheus.NewDesc(
		...
		[]string{"host", "domain", "target_device", "source_file", "serial", "bus", "disk_type", "driver_type", "cache", "discard"},
		nil)
	libvirtDomainBlockRdBytesDesc = prometheus.NewDesc(
		...
		[]string{"host", "domain", "target_device"},
		nil)
	...
	libvirtDomainMetaInterfacesDesc = prometheus.NewDesc(
		...
		[]string{"host", "domain", "source_bridge", "target_device", "virtual_interface"},
		nil)
...
func CollectStoragePool(ch chan<- prometheus.Metric, pool libvirt.StoragePool, hostName string) error {
	// Refresh pool
	err := pool.Refresh(0)
	if err != nil {
		return err
...
func CollectFromLibvirt(ch chan<- prometheus.Metric, uri string) error {
	...
	ch <- prometheus.MustNewConstMetric(
		libvirtNodeCpu,
		prometheus.GaugeValue,
		float64(nodeInfo.GetMaxCPUs()),
		hostName)

	ch <- prometheus.MustNewConstMetric(
		libvirtNodeMemory,
		prometheus.GaugeValue,
		float64(nodeInfo.Memory * 1024),
		hostName)
	...
	for _, pool := range pools {
		err = CollectStoragePool(ch, pool, hostName)
		pool.Free()
		if err != nil {
			return err
		}
